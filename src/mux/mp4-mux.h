 ...
 *
 * Adds a QuickTime timecode (tmcd) track. The start timecode is stored as a
 * frame count (real frames at fps_num/fps_den, i.e. drop-frame counting when
 * drop_frame is set). Must be called before the first fragment is flushed,
 * i.e. before the second keyframe is submitted. */
bool mp4_mux_set_timecode(struct mp4_mux *mux, uint32_t start_frame, uint32_t fps_num, uint32_t fps_den,
			  bool drop_frame);

/* Only start a new fragment on a keyframe that is at least `usec` after the
 * previous fragment boundary. 0 keeps OBS's default (every keyframe). */
