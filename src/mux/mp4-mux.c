...
static size_t mp4_write_tkhd(struct mp4_mux *mux, struct mp4_track *track)
{
	...
	/* Flags are 0x1 (enabled) | 0x2 (in movie). The timecode track is
	 * written "in movie" only, matching FFmpeg's layout (verified in
	 * DaVinci Resolve). */
	const uint32_t flags = track->type == TRACK_TIMECODE ? 0x2 : (0x1 | 0x2);
	...
	if (track->type == TRACK_AUDIO || track->type == TRACK_TIMECODE || !track->encoder) {
		s_wb32(s, 0); // width
		s_wb32(s, 0); // height
	} else {
...
static size_t mp4_write_mdhd(struct mp4_mux *mux, struct mp4_track *track)
{
	...
	if (track->type == TRACK_VIDEO || track->type == TRACK_TIMECODE) {
...
static size_t mp4_write_hdlr(struct mp4_mux *mux, struct mp4_track *track)
{
	...
	if (!track)
		...
		s_write(s, "text", 4);
	else if (track->type == TRACK_TIMECODE)
		s_write(s, "tmcd", 4);
	else
		s_write(s, "soun", 4);
	...
	if (!track)
		...
		handler_name = "OBS Timecode Handler";
	else
...
/// (QTFF/Apple) Timecode media information atom
static size_t mp4_write_tcmi(struct mp4_mux *mux)
{
	struct serializer *s = mux->serializer;
	int64_t start = serializer_get_pos(s);

	write_fullbox(s, 0, "tcmi", 0, 0);

	s_wb16(s, 0);      // text font
	s_wb16(s, 0);      // text face
	s_wb16(s, 12);     // text size
	s_wb16(s, 0);      // reserved
	s_wb16(s, 0);      // text colour r
	s_wb16(s, 0);      // text colour g
	s_wb16(s, 0);      // text colour b
	s_wb16(s, 0xffff); // background colour r
	s_wb16(s, 0xffff); // background colour g
	s_wb16(s, 0xffff); // background colour b

	static const char font_name[] = "Lucida Grande";
	s_w8(s, (uint8_t)(sizeof(font_name) - 1)); // pascal string
	s_write(s, font_name, sizeof(font_name) - 1);

	return write_box_size(s, start);
}

/// (QTFF/Apple) Base media information header atom for timecode tracks
static size_t mp4_write_gmhd_tmcd(struct mp4_mux *mux)
{
	struct serializer *s = mux->serializer;
	int64_t start = serializer_get_pos(s);
	...
	int64_t tmcd_start = serializer_get_pos(s);
	write_box(s, 0, "tmcd");
	mp4_write_tcmi(mux);
	write_box_size(s, tmcd_start);

	return write_box_size(s, start);
}

/// (QTFF/Apple) Timecode sample description
static size_t mp4_write_tmcd_entry(struct mp4_mux *mux)
{
	struct serializer *s = mux->serializer;
	int64_t start = serializer_get_pos(s);
	...
	/* Flags: 0x1 = drop frame */
	s_wb32(s, mux->tc_drop_frame ? 0x1 : 0x0);
	s_wb32(s, mux->tc_fps_num); // timescale
	s_wb32(s, mux->tc_fps_den); // frame duration

	/* Number of frames per second, rounded (60 for 59.94). */
	uint32_t nb_frames = (mux->tc_fps_num + mux->tc_fps_den / 2) / mux->tc_fps_den;
	s_w8(s, (uint8_t)nb_frames);
	s_w8(s, 0);   // reserved
	s_wb16(s, 0); // reserved
...
/// (QTFF/Apple) Timecode track reference
static size_t mp4_write_tref_tmcd(struct mp4_mux *mux)
{
	struct serializer *s = mux->serializer;
	int64_t start = serializer_get_pos(s);
	...
	s_wb32(s, mux->timecode_track->track_id);

	return write_box_size(s, start);
}

/// 8.3.3 Track Reference Box
static size_t mp4_write_tref(struct mp4_mux *mux, struct mp4_track *track)
{
	struct serializer *s = mux->serializer;

	bool chap = mux->chapter_track && (track->type == TRACK_VIDEO || track->type == TRACK_AUDIO);
	bool tmcd = mux->timecode_track && track->type == TRACK_VIDEO;
...
static size_t mp4_write_trak(struct mp4_mux *mux, struct mp4_track *track, bool fragmented)
{
	...
	// edts (not used for the timecode track)
	if (track->type != TRACK_TIMECODE)
		mp4_write_edts(mux, track);

	// tref
	mp4_write_tref(mux, track);
...
static size_t mp4_write_mvex(struct mp4_mux *mux)
{
	...
	if (mux->timecode_track)
		mp4_write_trex(mux, mux->timecode_track->track_id);
...
static size_t mp4_write_moov(struct mp4_mux *mux, bool fragmented)
{
	...
	if (mux->timecode_track)
		mp4_write_trak(mux, mux->timecode_track, fragmented);
...
static size_t mp4_write_moof(struct mp4_mux *mux, uint32_t moof_size, int64_t moof_start)
{
	...
	/* Timecode sample (first fragment only) */
	if (mux->timecode_track && mux->timecode_track->fragment_samples.num)
		mp4_write_traf(mux, mux->timecode_track, moof_start, moof_size, &samples_mdat_offset);
...
/* Creates the single 4-byte timecode sample (big-endian frame number) and
 * adds it to the fragment currently being built. */
static void queue_timecode_sample(struct mp4_mux *mux, uint64_t *mdat_size)
{
	struct mp4_track *track = mux->timecode_track;

	/* Packet data is preceded by a refcount so it can be released with
	 * obs_encoder_packet_release() like any other packet. */
	uint8_t *buf = bmalloc(sizeof(long) + 4);
	*(long *)buf = 1;
	uint8_t *data = buf + sizeof(long);
	data[0] = (uint8_t)(mux->tc_start_frame >> 24);
	data[1] = (uint8_t)(mux->tc_start_frame >> 16);
	data[2] = (uint8_t)(mux->tc_start_frame >> 8);
	data[3] = (uint8_t)(mux->tc_start_frame);

	struct encoder_packet pkt = {0};
	pkt.data = data;
	pkt.size = 4;
	pkt.timebase_num = track->timebase_num;
	pkt.timebase_den = track->timebase_den;
	...
	track->samples = 1;
	da_push_back(track->sample_sizes, &smp->size);

	*mdat_size += 4;
	mux->tc_sample_queued = true;
}
...
static void mp4_flush_fragment(struct mp4_mux *mux)
{
	...
	if (mux->timecode_track && !mux->tc_sample_queued)
		queue_timecode_sample(mux, &mdat_size);
	...
	if (mux->timecode_track)
		write_packets(mux, mux->timecode_track);
...
void mp4_mux_destroy(struct mp4_mux *mux)
{
	...
	free_track(mux->timecode_track);
	bfree(mux->timecode_track);
	da_free(mux->tracks);
	bfree(mux);
}
...
bool mp4_mux_submit_packet(struct mp4_mux *mux, struct encoder_packet *pkt)
{
	...
	if (type == OBS_ENCODER_AUDIO) {
		 ...
		 * (and, if configured, the minimum fragment duration has
		 * passed since the previous fragment boundary). */
		if (parsed_packet.keyframe && parsed_packet.pts > 0) {
			int64_t pts_usec = packet_pts_usec(&parsed_packet);
			if (!mux->min_frag_usec || pts_usec - mux->last_frag_pts_usec >= mux->min_frag_usec) {
				mux->next_frag_pts = pts_usec;
				mux->last_frag_pts_usec = pts_usec;
			}
		}
	}
...
bool mp4_mux_finalise(struct mp4_mux *mux)
{
	...
	/* Timecode track: one sample spanning the whole video duration. */
	if (mux->timecode_track && mux->timecode_track->chunks.num) {
		struct mp4_track *tc = mux->timecode_track;
		uint64_t duration = 0;

		for (size_t i = 0; i < mux->tracks.num; i++) {
			struct mp4_track *track = &mux->tracks.array[i];
			if (track->type == TRACK_VIDEO) {
				duration = util_mul_div64(track->duration, tc->timebase_den, track->timebase_den);
				break;
			}
		}
		...
		da_clear(tc->deltas);
		struct sample_delta *delta = da_push_back_new(tc->deltas);
		delta->count = 1;
		delta->delta = (uint32_t)duration;
	}
...
/* ===========================================================================*/
/* ISO Recorder additions */

bool mp4_mux_set_timecode(struct mp4_mux *mux, uint32_t start_frame, uint32_t fps_num, uint32_t fps_den,
			  bool drop_frame)
{
	if (mux->fragments_written) {
		warn("Timecode must be set before the first fragment is written");
		return false;
	}
	if (!fps_num || !fps_den)
		return false;
	...
	if (!mux->timecode_track) {
		struct mp4_track *track = bzalloc(sizeof(struct mp4_track));
		track->type = TRACK_TIMECODE;
		track->codec = CODEC_TMCD;
		track->track_id = ++mux->track_ctr;
		mux->timecode_track = track;
	}

	struct mp4_track *track = mux->timecode_track;
	track->timebase_num = fps_den;
	track->timebase_den = fps_num;
	track->timescale = fps_num;

	mux->tc_start_frame = start_frame;
	mux->tc_fps_num = fps_num;
	mux->tc_fps_den = fps_den;
	mux->tc_drop_frame = drop_frame;
	return true;
}

void mp4_mux_set_min_fragment_duration(struct mp4_mux *mux, int64_t usec)
{
	mux->min_frag_usec = usec > 0 ? usec : 0;
}
