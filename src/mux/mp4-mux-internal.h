...
enum mp4_track_type {
	...
	TRACK_AUDIO,
	TRACK_CHAPTERS,
	/* ISO Recorder: QuickTime timecode track */
	TRACK_TIMECODE,
};
...
enum mp4_codec {
	...
	/* ISO Recorder: QuickTime timecode */
	CODEC_TMCD,
};
...
struct mp4_mux {
	...
	/* QuickTime timecode (tmcd) track, written into the initial moov
	 * and the first fragment so it is readable while the file grows. */
	struct mp4_track *timecode_track;
	uint32_t tc_start_frame;
	uint32_t tc_fps_num;
	uint32_t tc_fps_den;
	bool tc_drop_frame;
	bool tc_sample_queued;
	/* Minimum distance between fragments (0 = fragment on every
	 * keyframe, which is OBS's default behaviour). */
	int64_t min_frag_usec;
	int64_t last_frag_pts_usec;
};
